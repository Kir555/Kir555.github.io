...
$('.dropdown a').click(function (e) {
  if (!$(this).hasClass('active')) {
    ...
    $(this).parent().parent().find('.dropdown-active-text').text(this.innerText);
...
function openTab(e, t, n) {
  ...
  if (t === 'tab1') {
    document.getElementById('dilveryType').innerText = 'Выберите адрес самовывоза';
  } else {
    document.getElementById('dilveryType').innerText = 'Выберите службу доставки';
  }
}
...
if (document.getElementById('changeSide')) {
  document.getElementById('changeSide').addEventListener('click', function () {
    console.log(document.getElementById('cardPhoto').getElementsByTagName('img'));

    if (this.classList.contains('back')) {
      [].map.call(document.getElementById('cardPhoto').getElementsByTagName('img'), function (el) {
        el.setAttribute('src', 'img/carpet-preview.jpg');
      });
    } else {
      [].map.call(document.getElementById('cardPhoto').getElementsByTagName('img'), function (el) {
        el.setAttribute('src', 'img/carpet-preview-back.jpg');
      });
    }
